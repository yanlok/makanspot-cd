...
import '../../home/models/restaurant_repository.dart';
...
final mapMarkersProvider = FutureProvider<Set<Marker>>((Ref ref) async {
  ...
  return restaurants.map((r) => Marker(
    ...
    infoWindow: InfoWindow(title: r.name, snippet: '${r.rating} stars • ${r.priceRange}'),
  )).toSet();
});
...
final filteredRestaurantsProvider = FutureProvider<List<RestaurantModel>>((Ref ref) async {
  ...
  final repo = ref.watch(restaurantRepositoryProvider);
  if (filter == 'Hidden Gems') return await repo.getHiddenGems();
  if (filter == 'Trending') return await repo.getTrendingRestaurants();
