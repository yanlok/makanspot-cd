...
export async function v2SetPrimaryImage(
  ...
  if (existing) {
    const { error } = await supabase.from("v2_restaurant_images").update({
    ...
    const { error } = await supabase.from("v2_restaurant_images").insert({
  ...
  const { error: demoteError } = await supabase.from("v2_restaurant_images")
  ...
  const { error: promoteError } = await supabase.from("v2_restaurant_images")
    .update({
      is_primary: true,
      source_url: sourceUrl,
      quality_score: qualityScore,
